...
void
bc_free_slip::solve(qk::variable::variable_manager & variable_manager, const int tag) const
{
        ...
        for(qk::indexer chunk_idx = var.indexer(); chunk_idx.exists(); chunk_idx.next()){
            ...
            for(const int & i : _dims){
                ...
                // Lower boundary
                {
                    ...
                    efield_rng.set(lower_external_range.num_dims()-1, 1,4);
                ...
                // Upper boundary
                {
                    ...
                    efield_rng.set(upper_external_range.num_dims()-1, 1,4);
                    chunk.multiply_by_scalar(efield_rng, -1.);
                }
            }
        }
