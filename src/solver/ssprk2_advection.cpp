...
void advection_iterate(const int start,
    const int end,
    const int downstream_stride,
    const double coeff,
    const double * __restrict__ n,
    double * __restrict__ np)
{
...
#pragma omp parallel for
    for (int i = start; i < end; i++) {
        // Center cell value
        const double vc = a[i - downstream_stride];
        // Left cell value
        const double vn = a[i - 2 * downstream_stride];
        // Right cell value
        const double vp = a[i];

        const double r = (vp != vc) ? (vc - vn) / (vp - vc) : 1.0; // 2 add, 1 div : 3 FLO

//        std::cout << "reading indexes (vc,vn,vp) = " << i - downstream_stride << ", " << i - 2 * downstream_stride << ", " << i << std::endl;
...
//            }
        ...
        b[i - downstream_stride] -= dflux; // 1 add : 1 FLO
...
void
advection_inout(const double dt, const std::vector<double> & velocity, const qk::grid::rectilinear & grid, const qk::data::extended_datachunk & a, qk::data::extended_datachunk & b)
{
    // Apply fluxes
    for (int dim = 0; dim < grid.num_dims(); dim++) {

        int downstream_stride, itr_start, itr_end;
        if(velocity[dim] > 0.){
            downstream_stride = a.stride(dim);
            itr_start = 2*a.stride(dim);
            itr_end = a.volume()-1;
        } else {
            downstream_stride = -a.stride(dim);
            itr_start = 0;
            itr_end = a.volume() - 2*a.stride(dim);
        }

//        std::cout << "advection dim " << dim << ", downstream_stride " << downstream_stride << ", start " << itr_start << ", end " << itr_end << std::endl;

        advection_solver::advection_iterate(itr_start, itr_end, downstream_stride, std::fabs(dt * velocity[dim] / grid.dx(dim)), a.data(), b.data());
    }
}


void advection(const qk::grid::rectilinear & grid,
...
void ssprk2_advection::setup(const std::vector<double> & velocity)
{
    _velocity = velocity;
}

void ssprk2_advection::solve(qk::variable::variable_manager & variable_manager, const int tag) const
{
...
void ssprk2_advection::stage_0(const double dt,
    const qk::grid::rectilinear & grid,
    const qk::data::extended_datachunk & n0,
    qk::data::extended_datachunk & n1) const
{

    // Copy n0 into n1 to initialize - uses memcpy vs the pull operator
    qk::data::full_copy(n0, n1);

    advection_solver::advection_inout(dt,_velocity,grid,n0,n1);
//
//
//
//
...
//        int downstream_stride, itr_start, itr_end;
//        if(_velocity[dim] > 0.){
//            downstream_stride = stride[dim];
//            itr_start = 2*n0.stride(dim);
//            itr_end = n0.volume()-1;
//        } else {
//            downstream_stride = -stride[dim];
//            itr_start = 0;
//            itr_end = n0.volume() - 2*stride[dim];
//        }
//
////        std::cout << "advection dim " << dim << ", downstream_stride " << downstream_stride << ", start " << itr_start << ", end " << itr_end << std::endl;
//
//        advection_solver::advection_iterate(itr_start, itr_end, downstream_stride, dt * _velocity[dim] / grid.dx(dim), n, ns);
//    }
}

void ssprk2_advection::stage_1(const double dt,
    const qk::grid::rectilinear & grid,
    const qk::data::extended_datachunk & n0,
    ...
    qk::data::extended_datachunk & n2) const
{
    ...
    advection_solver::advection_inout(0.5*dt,_velocity,grid,n1,n2);
...
////        std::cout << "advection dim " << dim << ", downstream_stride " << downstream_stride << ", start " << itr_start << ", end " << itr_end << std::endl;
//
//            advection_solver::advection_iterate(itr_start, itr_end, downstream_stride, 0.5*dt * _velocity[dim] / grid.dx(dim), ns, np);
//    }
//
