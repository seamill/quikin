...
void
bc_conducting_wall::solve(qk::variable::variable_manager & variable_manager, const int tag) const
{
    ...
    for(int vi = 0; vi < _output_variable_ids.size(); vi++){
        ...
        for(qk::indexer chunk_idx = var.indexer(); chunk_idx.exists(); chunk_idx.next()){
            ...
            for(const int & i : _dims){
                ...
                // Lower boundary
                {
                    qk::range efield_rng(lower_external_range);
                    efield_rng.set(lower_external_range.num_dims()-1, 0,4);
                    chunk.multiply_by_scalar(efield_rng, -1.);
                }

                // Upper boundary
                {
                    qk::range efield_rng(upper_external_range);
                    efield_rng.set(upper_external_range.num_dims()-1, 0,4);
                    chunk.multiply_by_scalar(efield_rng, -1.);
                }
            }
        }
